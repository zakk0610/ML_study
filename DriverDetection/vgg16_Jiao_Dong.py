# -*- coding: utf-8 -*-
# https://www.kaggle.com/c/state-farm-distracted-driver-detection/forums/t/20747/simple-lb-0-23800-solution-keras-vgg-16-pretrained
# https://www.kaggle.com/jiaodong/state-farm-distracted-driver-detection/vgg-16-pretrained-loss-0-23800/comments
...
def get_im(path, img_rows, img_cols, color_type=1):
    ...
    # resized = resized.astype(np.float16, copy=False)
...
def load_train(img_rows, img_cols, color_type=1):
    ...
    # driver[picutre.jpg] = driver_id
    driver_data = get_driver_data()

    print('Read train images')
    for j in range(10):
        print('Load folder c{}'.format(j))
        path = os.path.join('.', 'picture', 'train',
                            'c' + str(j), '*.jpg')
...
def read_and_normalize_and_shuffle_train_data(img_rows, img_cols,
    ...
    for c in range(color_type_global):
        train_data[:, c, :, :] = train_data[:, c, :, :] - mean_pixel[c]
    # train_data /= 255

    # ignore permuatation by zakk
...
def vgg_std16_model(img_rows, img_cols, color_type=1):
    ...
    model.load_weights('web_vgg16_weights.h5')
