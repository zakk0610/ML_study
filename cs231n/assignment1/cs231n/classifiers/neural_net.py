...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    #input - fully connected layer - ReLU - fully connected layer - softmax
    #print X.shape, W1.shape, W2.shape # (5, 4) (4, 10) (10, 3)  
    hidden_layer = np.dot(X, W1) + b1
    relu_layer = np.maximum(hidden_layer, 0)
    ...
    data_loss = np.sum(-1 * scores[range(N), y]) + np.sum(np.log(np.sum(exp_scores, axis=1)))
    data_loss /= N
    ...
    #print X.shape, W1.shape, W2.shape # (5, 4) (4, 10) (10, 3)
    #print relu_layer # (5, 10)
    #print dscores.shape #(5, 3)
    #print b2.shape # (3, )
    grads['W2'] = np.dot(relu_layer.T, dscores)
    grads['b2'] = np.sum(dscores, axis=0)
    
    drelu_layer = np.dot(dscores, W2.T)
    
    #print dhidden_layer.shape #(5, 10)
    dhidden_layer = drelu_layer  
  ...
  def train(self, X, y, X_val, y_val,
            ...
            batch_size=200, verbose=False, epoches=1):
    """
